...
export default function AdminPage() {
  ...
  async function handleBulkAddKeys() {
    ...
    if (!error) { setBulkKeys({ productId: "", keysText: "" }); alert(`เพมิ่ สำเรจ็ ${keys.length} คีย`); }
  }
  ...
  if (!isAdmin) return (
    <div className="max-w-md mx-auto px-4 py-20 text-center">
      <h2 className="text-xl font-semibold text-red-400 mb-2">ไมม ีสทิ ธิเ้ ขาถงึ </h2>
      <p className="text-slate-500">คณุ ไมม ีสทิ ธิเ์ พอื่ เขาถงึ หนาน ี</p>
    </div>
  );

  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold neon-text mb-6">Panel ผด ู แลระบบ</h1>
      <div className="flex gap-2 mb-8 border-b border-slate-800">
        {(["overview", "products", "keys"] as const).map((tab) => (
          ...
          >{tab === "overview" ? "ภาพรวม" : tab === "products" ? "ส นิ คา" : "คีย"}</button>
        ))}
      </div>

      {activeTab === "overview" && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard icon={<DollarSign className="text-green-400" />} label="รายไดรวม" value={`฿${stats.totalRevenue.toFixed(2)}`} />
          <StatCard icon={<Package className="text-cyan-400" />} label="จำนว นการซอื้ " value={stats.totalSales.toString()} />
          <StatCard icon={<Users className="text-purple-400" />} label="จำนว นผใู ้ช" value={stats.totalUsers.toString()} />
          <StatCard icon={<Key className="text-cyan-400" />} label="สนิ คาท ัง้ หมด" value={stats.totalProducts.toString()} />
        </div>
      )}

      {activeTab === "products" && (
        <div className="max-w-xl">
          <h2 className="text-lg font-semibold text-slate-100 mb-4 flex items-center gap-2"><Plus size={18} className="text-cyan-400" /> เพมิ่ ส นิ คาใหม</h2>
          <form onSubmit={handleAddProduct} className="space-y-4">
            <div>
              <label className="block text-sm text-slate-400 mb-1">ชอื่ ส นิ คา</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-slate-400 mb-1">หมวดหมู</label>
            ...
            <div>
              <label className="block text-sm text-slate-400 mb-1">ลิงกร ู ป</label>
            ...
            <button type="submit" className="w-full py-2.5 bg-cyan-400 text-slate-950 font-medium rounded-lg hover:bg-cyan-300 transition-all">เพมิ่ ส นิ คา</button>
          </form>
        </div>
      )}
      ...
      {activeTab === "keys" && (
        <div className="max-w-xl">
          <h2 className="text-lg font-semibold text-slate-100 mb-4 flex items-center gap-2"><Key size={18} className="text-purple-400" /> เพมิ่ คียแบบจำนวนมาก</h2>
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-slate-400 mb-1">เลอื กส นิ คา</label>
              <select value={bulkKeys.productId} onChange={(e) => setBulkKeys({ ...bulkKeys, productId: e.target.value })} className="w-full px-4 py-2 bg-slate-900 border border-slate-700 rounded-lg text-slate-100 focus:border-purple-500/50 transition-colors">
                <option value="">-- เลอื กส นิ คา --</option>
                {products.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm text-slate-400 mb-1">วางคีย (บรรทดั ละ 1 คีย)</label>
            ...
            <button onClick={handleBulkAddKeys} disabled={!bulkKeys.productId || !bulkKeys.keysText.trim()} className="w-full py-2.5 bg-purple-600 text-white font-medium rounded-lg hover:bg-purple-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed">เพมิ่ คีย</button>
          </div>
        </div>
      )}
    </div>
