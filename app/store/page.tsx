...
const CATEGORIES = ["ทัง้ หมด", "Blox Fruits", "King Legacy", "Pet Simulator 99", "Blade Ball", "อนื่ ๆ"];

export default function StorePage() {
  const { supabase, user } = useSupabase();
  const [products, setProducts] = useState<Product[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedCategory, setSelectedCategory] = useState("ทัง้ หมด");
  ...
  async function handleBuy(product: Product) {
    if (!user) { setBuyStatus({ status: "error", message: "กรณาุ เขาสส ู ystem เพอื่ ซอื้ สนิ คา" }); return; }
    ...
    if (error || !data || data.length === 0) {
      setBuyStatus({ status: "error", message: error?.message ?? "การซอื้ ลมเหลว" });
    } else {
      const result = data[0];
  ...
  const filtered = products.filter((p) => {
    const matchCat = selectedCategory === "ทัง้ หมด" || p.game_category === selectedCategory;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold neon-text mb-1">รานสคริปต</span>
          <p className="text-slate-400 text-sm">คนหาและซอื้ สคริปต Roblox คณุ ภาพ</p>
        </div>
        <div className="relative">
          <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
          <input
            type="text"
            placeholder="คนหาสคริปต..."
            value={searchQuery}
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden neon-border card-hover flex flex-col">
      <div className="relative h-40 bg-slate-800">
        ...
        <div className="absolute top-2 right-2 px-2 py-1 rounded text-xs font-mono bg-slate-950/80 text-slate-300 border border-slate-700">เหลือ {product.stock_count} อ ัน</div>
      </div>
      <div className="p-5 flex-1 flex flex-col">
        ...
        <div className="flex items-center justify-between mt-auto">
          ...
          {buyStatus?.key && buyStatus.status === "success" ? (
            <button onClick={() => navigator.clipboard.writeText(buyStatus.key!)} className="px-4 py-2 text-sm font-medium bg-green-500/20 text-green-400 border border-green-500/50 rounded-lg hover:bg-green-500/30 transition-all">คดั ลอกคีย</button>
          ) : (
            ...
            >
              {isBuying ? <><div className="w-4 h-4 border-2 border-slate-950 border-t-transparent rounded-full animate-spin" />กำลงั ดำเนนิ การ...</> : !inStock ? "สนิ คาหมด" : <><ShoppingCart size={14} />ซอื้ เลย</>}
            </button>
          )}
        </div>
