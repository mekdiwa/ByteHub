  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <h1 className="text-3xl font-bold neon-text text-center mb-2">สรางบ ั ญช ี</h1>
        <p className="text-slate-400 text-center mb-8">รวมก ั บ ByteHub แลวเริม่ ตนสำรวจ</p>
        <form onSubmit={handleSignup} className="space-y-4">
          <div>
            <label className="block text-sm text-slate-400 mb-1">ชอื่ ผใู ้ช</label>
          ...
          <button type="submit" disabled={loading} className="w-full py-2.5 bg-cyan-400 text-slate-950 font-semibold rounded-lg hover:bg-cyan-300 transition-all glow-cyan flex items-center justify-center gap-2">
            {loading ? <Loader2 size={18} className="animate-spin" /> : "สรางบ ั ญช ี"}
          </button>
        </form>
        <p className="text-center text-slate-500 text-sm mt-6">ม ีบ ั ญช ีแลว? <Link href="/login" className="text-cyan-400 hover:underline">เขาสส ู ystem</Link></p>
      </div>
    </div>
  );
}
